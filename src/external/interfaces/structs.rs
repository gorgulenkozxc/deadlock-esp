use super::{
    entities::Player,
    enums::{AbilitySlot, Hero, TargetBone},
    math::Vector3,
};
use crate::{
    external::{
...
impl GameSceneNode {
    pub fn update(&mut self, pawn_ptr: *mut c_void) {
        unsafe {
            let game_scene_node: *mut c_void =
                read_memory(pawn_ptr.add(C_BaseEntity::m_pGameSceneNode));
            ...
            self.view_angle_y =
                read_memory(game_scene_node.add(CGameSceneNode::m_angAbsRotation + 4usize));
        }
    }
}
...
impl AbilitiesComponent {
    pub fn update(&mut self, entity_list_ptr: *mut c_void, pawn_ptr: *mut c_void) {
        ...
        unsafe {
            ...
            let vec_abilities: *mut c_void =
                read_memory(ability_component.add(CCitadelAbilityComponent::m_vecAbilities + 0x8));
            for i in 0..22 {
                ...
                self.list.push(Ability {
                    ...
                    coodown: read_memory(
                        ability.add(C_CitadelBaseAbility::m_bIsCoolingDownInternal),
                    ),
    ...
    pub fn get(&self, slot: AbilitySlot) -> Option<&Ability> {
        self.list.iter().find(|&ability| ability.slot == slot)
    }
}
        ...
        unsafe {
            if local_player.is_alive() {
                for player in players.iter() {
                    ...
                    let player_obs_target_pawn =
                        Self::get_obs_target_pawn(entity_list_ptr, player.pawn.ptr);
                    if local_player.pawn.ptr == player_obs_target_pawn {
                        self.spectator_list.push(format!("{:?}", player.data.hero));
                    }
                }
            } else {
                let local_player_obs_target_pawn =
                    Self::get_obs_target_pawn(entity_list_ptr, local_player.pawn.ptr);
                for player in players.iter() {
                    if player.is_alive() || player.pawn.ptr == local_player.pawn.ptr {
                        continue;
                    }
...
impl Camera {
    pub fn update(&mut self, client_ptr: *mut c_void) {
        unsafe {
            let camera_ptr: *mut c_void =
                read_memory(client_ptr.add(dwCCitadelCameraManager + 0x28));
...
impl PlayerDataGlobal {
    pub fn update(&mut self, controller_ptr: *mut c_void) {
        unsafe {
            let struct_offset: *mut c_void =
                controller_ptr.add(CCitadelPlayerController::m_PlayerDataGlobal);
            ...
            self.ult_cd_time_end =
                read_memory(struct_offset.add(PlayerDataGlobal_t::m_flUltimateCooldownEnd));
...
impl Default for Skeleton {
    fn default() -> Self {
        Self {
            ...
            bone_array: std::ptr::null_mut::<c_void>(),
    ...
    pub fn update(&mut self, pawn_ptr: *mut c_void, hero: Hero, target_bone: &TargetBone) {
        unsafe {
            let game_scene_node: *mut c_void =
                read_memory(pawn_ptr.add(C_BaseEntity::m_pGameSceneNode));
            self.bone_array =
                read_memory(game_scene_node.add(CSkeletonInstance::m_modelState + 0x80));
                    ...
                    Some(i) => {
                        if i != -1 {
                            ...
                            if *target_bone == TargetBone::Neck {
                                ...
                                self.target_bone_pos = read_memory(
                                    self.bone_array
                                        .add(bone.get_bone_index(hero) as usize * 32usize),
                                );
...
impl Controller {
    ...
    pub fn update(&mut self, base_ptr: *mut c_void, index: i32) {
        unsafe {
            ...
            self.local = read_memory(
                self.ptr
                    .add(CBasePlayerController::m_bIsLocalPlayerController),
            );
        }
    }
}
