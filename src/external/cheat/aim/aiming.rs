            ...
            None => {
                if let Some(entity_index) = get_entity_array_index() {
                    ...
                    if settings.creeps.velocity_prediction {
                        target_pos =
                            calc_velocity(target_pos, entity.pawn.velocity, &settings.players);
                    }
...
pub fn find_player(game: &External, local_player: &Player, settings: &AimProperties) {
    ...
    for p in game.players.iter() {
        if p.pawn.team != game.get_local_player().pawn.team && p.is_alive() {
            let mut target_pos = p.skeleton.target_bone_pos;
            if game.view_matrix.transform(&mut target_pos)
                && in_fov(target_pos, center, settings.fov)
            {
                let cur_distance = Vector3::distance_2d(target_pos, Vector3::from_pos2(center));
                if cur_distance < distance
                    && Vector3::distance(
                        p.game_scene_node.position,
...
fn find_entity(
    ...
    for ent in game.entities.iter() {
        ...
        if ent.class.as_priority_2(global_settings.priority) >= priority {
            ...
            let mut head_pos = ent.game_scene_node.position;
            ...
            if !ent.game_scene_node.dormant
                && game.view_matrix.transform(&mut head_pos)
                && in_fov(head_pos, center, settings.fov)
            {
...
fn aim_to(
    ...
    let aim_direction_desired = Vector3::normalize(
        point_world
            - calc_velocity(
                game.camera.position,
                game.get_local_player().pawn.velocity,
                settings,
            ),
    );
...
fn get_aim_direction(client_ptr: *mut c_void, punch: Vector3) -> Vector3 {
    ...
    vector_from_euler_angles(
        camera.view_angles.x.to_radians(),
        camera.view_angles.y.to_radians(),
    )
}
...
fn angle_to_signed(vec1: Vector3, vec2: Vector3, about: Vector3) -> f32 {
    ...
    let sign = Vector3::dot(
        Vector3::normalize(Vector3::cross(vector_on_plane, other_on_plane)),
        plane.normal,
    );
