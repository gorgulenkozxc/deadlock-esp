...
use crate::{
    external::{
        ...
        offsets::client_dll::{CCitadel_Ability_PrimaryWeapon, CCitadel_Ability_Slide},
    },
    input::keyboard::{self, KeyState, VirtualKeys},
    memory::read_memory,
    settings::structs::Priority,
};
...
impl HeroScript for EntityPriorityToggle {
    fn update(
        &mut self,
        _: &crate::external::External,
        key_state: KeyState,
        settings: &mut crate::settings::structs::Settings,
    ) {
...
impl HeroScript for AutoReload {
    fn update(
        &mut self,
        game: &crate::external::External,
        _: KeyState,
        _: &mut crate::settings::structs::Settings,
    ) {
        let local_player = game.get_local_player();
        let ult_ability = local_player
            .abilities
            .get(AbilitySlot::ESlot_Weapon_Primary);
        if ult_ability.is_none() {
            return;
        }
        let ult_ability = ult_ability.unwrap();
        unsafe {
            let in_reload: bool = read_memory(
                ult_ability
                    .ptr
                    .add(CCitadel_Ability_PrimaryWeapon::m_bInReload),
            );
            if in_reload && !self.reloaded {
                let can_active_reload: bool = read_memory(
                    ult_ability
                        .ptr
                        .add(CCitadel_Ability_PrimaryWeapon::m_bCanActiveReload),
                );
                if can_active_reload {
                    let cur_time = game.global_vars.current_time;
                    let reload_start: f32 = read_memory(
                        ult_ability
                            .ptr
                            .add(CCitadel_Ability_PrimaryWeapon::m_flNextAttackDelayStartTime),
                    );
                    let reload_end: f32 = read_memory(
                        ult_ability
                            .ptr
                            .add(CCitadel_Ability_PrimaryWeapon::m_flNextAttackDelayEndTime),
                    );
                    let reload_pause: f32 = read_memory(
                        ult_ability
                            .ptr
                            .add(CCitadel_Ability_PrimaryWeapon::m_flAttackDelayPauseTotalTime),
                    );
                    let reload_time = (reload_end - reload_start) / 2f32;
                    // Это немного костыльный способ. Значения подбирал сам на глаз.
                    if cur_time - game.game_rules.paused_time
                        > reload_start + (reload_time - 0.15f32) + (reload_pause / 2f32)
                    {
                        self.reloaded = true;
    ...
    unsafe fn update_slide(&mut self, local_player: &Player, ability_slide: &Ability) {
        ...
        if sliding {
            let vel = (f32::abs(local_player.pawn.velocity.x).powf(2f32)
                ...
                + f32::abs(local_player.pawn.velocity.z).powf(2f32))
            .sqrt();
...
impl HeroScript for MovementScript {
    fn update(
        ...
        game: &crate::external::External,
        key_state: KeyState,
        _settings: &mut crate::settings::structs::Settings,
    ) {
        let local_player = game.get_local_player();
