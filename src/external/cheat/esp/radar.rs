...
fn draw_player(
    ...
    if settings.icons {
        ...
        draw_direction(
            ...
            radar_pos.y,
            player_view_angle,
            settings.player_radius * 3f32,
            color,
        );
        ui.painter()
            .circle_filled(radar_pos, settings.player_radius, color);
    }
}
...
fn draw_direction(
    ...
    let triangle_points = vec![arrow, arrow_l, arrow_r];
    g.add(PathShape::convex_polygon(
        triangle_points,
        color,
        Stroke::default(),
    ));
