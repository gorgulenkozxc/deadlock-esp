...
pub fn draw(ui: &mut Ui, player: &Player, settings: &HealthbarSettings) {
    ...
    let icon_pos = pos2(
        hp_rect.min.x - icon_size,
        hp_rect.min.y + height / 2f32 - icon_size / 2f32,
    );
...
fn get_hpbar_rect(player_rect: Rect, hp: i32, hp_max: i32) -> Rect {
    Rect {
        min: Pos2 {
            x: player_rect.min.x,
            y: player_rect.max.y + OFFSET,
        },
        max: Pos2 {
            x: player_rect.min.x + player_rect.width() / hp_max as f32 * hp as f32,
            y: player_rect.max.y + OFFSET + get_height(player_rect.width()),
        },
    }
}
