...
use crate::{
    ...
    settings::structs::HealthbarSettings,
};
...
pub fn draw(
    ...
    local_player: &Player,
    matrix: &Matrix,
    settings: &crate::settings::structs::Settings,
    ord: usize,
) {
    ...
    let circle_pos = pos2(
        (dir.x * radius) + screen_center.x,
        (dir.y * radius) + screen_center.y,
    );

    let icon_rect = Rect {
        min: pos2(
            circle_pos.x - icon_size / 2f32,
            circle_pos.y - icon_size / 2f32,
        ),
        max: pos2(
            (circle_pos.x - icon_size / 2f32) + icon_size,
            (circle_pos.y - icon_size / 2f32) + icon_size,
        ),
    };
    ...
    if settings.offscreen.enable_icon {
        ...
        if behind {
            image
                .tint(Color32::from_rgba_unmultiplied(200, 200, 200, 100))
...
fn draw_healthbar(g: &Painter, player: &Player, icon_rect: Rect, settings: &HealthbarSettings) {
    ...
    let hp_rect = Rect {
        ...
        max: pos2(
            rect_hp_max.min.x
                + rect_hp_max.width() / player.data.max_health as f32 * player.data.health as f32,
            rect_hp_max.max.y,
        ),
    };
...
fn draw_distance(g: &Painter, icon_rect: Rect, player: &Player, local_player: &Player) {
    ...
    let distance_m = (Vector3::distance(
