...
use crate::{
    input::keyboard::KeyState,
    memory::{self, read_memory},
    settings::structs::Settings,
};
use cheat::{
    aim,
    esp::{boxes::draw_head, offscreen, radar},
    scripts::{HeroScript, HeroScriptSettings},
};
use egui::{Pos2, Ui};
use egui_notify::Toasts;
use interfaces::{
    entities::{Entity, Player},
    enums::{Hero, TargetBone},
    math::{Matrix, Vector3},
...
impl External {
    pub fn new() -> Self {
        unsafe {
            ...
            let players: [Player; PLAYERS_LEN] = [
                ...
                Player::new(11),
                Player::new(12),
                Player::new(13), // на всякий еще добавил
            ];
            Self {
                client_ptr,
    ...
    pub unsafe fn update(
        ...
        unsafe {
            ...
            let viewport = Matrix::get_viewport(egui::Vec2 {
                x: self.screen.max.x,
                y: self.screen.max.y,
            });
                ...
                match &mut hero_script.1.key {
                    Some(key) => {
                        ...
                        hero_script
                            ...
                            .lock()
                            .unwrap()
                            .update(game, key.state, settings);
                    }
                    None => {
                        hero_script
                            .0
                            .lock()
                            .unwrap()
                            .update(game, KeyState::None, settings);
                    }
                }
            }
        }
    ...
    fn update_observers(&mut self) {
        ...
        self.observers
            .update(self.entity_list_ptr, local_player, &self.players);
    }
    ...
    fn update_entities(&mut self) {
        ...
        let count = if self.last_ent_aim_time.elapsed().as_secs_f32() > 3f32 {
            ...
            unsafe {
                read_memory::<i32>(
                    self.entity_list_ptr
                        .add(offsets::client::highestEntityIndex),
                )
            }
        };
        ...
        unsafe {
            for i in 0..count {
                entities.push(Entity::new(
                    i,
                    memory::read_memory(self.client_ptr.add(offsets::client::dwEntityList)),
                ));
            }
        }
    ...
    fn update_players(&mut self, target_bone: &TargetBone) {
        ...
        if self.local_player_index != 0 {
            ...
            local_player
                .abilities
                .update(self.entity_list_ptr, local_player.pawn.ptr);
            unsafe {
                let camera_services: *mut c_void = read_memory(
                    local_player
                        .pawn
                        .ptr
                        .add(C_BasePlayerPawn::m_pCameraServices),
                );
                self.aim_punch =
                    read_memory(camera_services.add(CPlayer_CameraServices::m_vecPunchAngle));
            }
        }
    }
            ...
            if settings.esp_players.render {
                for player in self.players.iter() {
                    if !player.is_invalid()
                        && player.is_alive()
                        && player.pawn.team != local_player.pawn.team
                    {
                ...
                if settings.offscreen.enable {
                    ...
                    for player in self.players.iter() {
                        p_indexes.push((
                            player.index,
                            Vector3::distance(player.game_scene_node.position, local_pos),
                        ));
                    }
...
impl GlobalVars {
    /// # Safety
    /// Placeholder
    pub unsafe fn update(&mut self, client_ptr: *mut c_void) {
        unsafe {
            let global_vars: *mut c_void =
                read_memory(client_ptr.add(offsets::client::dwGlobalVars));
...
impl GameRules {
    /// # Safety
    /// Placeholder
    pub unsafe fn update(&mut self, client_ptr: *mut c_void, interval: f32) {
        unsafe {
